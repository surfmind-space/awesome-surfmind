...
1. Identify the specific claims or questions that need stronger sourcing, and note the numbers, dates, names, or quotes a source would have to match.
2. Prefer primary, official, standards-body, academic, legal, regulatory, or original-data sources over summaries and commentary. Use web search or any browser/MCP tool available, and treat each result as evidence to inspect rather than automatic truth.
3. Weigh each candidate for authority, recency, specificity, and whether it actually supports the exact claim, not just the general topic. Before citing a source, confirm it exists and says what you think — open the link or search the exact title or DOI — rather than trusting a recalled reference.
4. Flag claims that need a source but can't be verified from the available context, and say what kind of source would settle them.
